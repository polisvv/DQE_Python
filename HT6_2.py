...
class Reader:
    ...
    def is_valid_publication_method(self, publication_method):
        if publication_method != "1" and publication_method != "2":
            print('Invalid publication method')
        return publication_method

    def get_publication_method(self):
        while True:
            publication_method = input("Please select publication method\n1 - via txt file\n2 - type via cli\n")
            if self.is_valid_publication_method(publication_method):
    ...
    def get_publication_type(self, publication_method, inp_list):
        if publication_method == "txt":
            # inp_list = pub_list_from_file
            return self.get_publication_type_from_file(inp_list) #, inp_list
    ...
    def is_valid_publication_type(self, publication_type):
    ...
    def get_publication_type_from_cli(self):
        while True:
            ...
            if self.is_valid_publication_type(publication_type):
    ...
    def get_publication_text(self, publication_method, inp_list):
        if publication_method == "txt":
            return self.get_publication_text_txt(inp_list)
        elif publication_method == "cli":
            return self.get_publication_text_cli()

    def get_publication_text_cli(self):
        body = input('Type text: ')
        return body

    def get_publication_text_txt(self, inp_list):
    ...
    def get_publication_specific_info(self, publication_method: str, publication, inp_list) -> str:
        if publication_method == "txt":
            return self.add_publication_specific_info_txt(publication, inp_list)
        elif publication_method == "cli":
            return self.add_publication_specific_info_cli(publication)

    def add_publication_specific_info_txt(self, publication: str, inp_list) -> str:
    ...
    def add_publication_specific_info_cli(self, publication: str) -> str:
        if publication.publication_type == "News":
...
class News:
    def __init__(self, publish_datetime, publication_type: str, location: str = "") -> None:
...
class Ad:
    def __init__(self, publication_type: str, expiration_date: str = "") -> None:
        self.publication_type = publication_type
...
class Joke:
    def __init__(self, publication_type: str, funny_meter: str = "") -> None:
        self.publication_type = publication_type
...
class Publisher:
    ...
    def write_to_list(self, publication, body, specific_info):
        publication_list = []
...
pub = Reader()
pub_method = pub.get_publication_method()
...
pub_spec_info = pub.get_publication_specific_info(pub_method, pub_type, pub_list_from_file)
file = Publisher()
file.write_to_file(norm.capitalize_and_append_sent(file.write_to_list(pub_type, pub_body, pub_spec_info)))
